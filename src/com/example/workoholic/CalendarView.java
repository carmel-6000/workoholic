...
public class CalendarView extends ImageView {
    ...
    HashMap<Integer, HashMap<String,Integer>> cellsData;
	...
	private void initCalendarView(Context context) {
		...
		initMonthlyData(context);		    
    }
	
	@SuppressLint("UseSparseArrays")
	protected void initMonthlyData(Context context)
	{
		WorkingSessionsIO wsIO = new WorkingSessionsIO(context);
		wsIO.open();
		List<WorkingSession> monthlyList = wsIO.getMonthlySessions();
		wsIO.close(); 
		Log.d(TAG,"getMonthlySessions results : ");
		cellsData = new HashMap<Integer, HashMap<String,Integer>>();
		for (WorkingSession ws : monthlyList)
		{
			final int currDay = ws.getDay();
			final int workingHours = ws.getSessionInterval();
		    Log.d(TAG,"beginTime : "+ws.getBeginTime());
		    Log.d(TAG,"day : "+Integer.toString(ws.getDay()));
		    Log.d(TAG,"\n");
		    if (cellsData.get(currDay) == null){
		    	cellsData.put(currDay,new HashMap<String,Integer>(){
		    ...
		    {
		    	cellsData.get(currDay).put("sessions", cellsData.get(currDay).get("sessions")+1);
		    	cellsData.get(currDay).put("workingHours",cellsData.get(currDay).get("workingHours")+2);
		    }	
		}
		Log.d(TAG,"wh HashMap -->"+cellsData.toString());
	}
			...
			for(int day=0; day<mCells[week].length; day++) {
				if(tmp[week][day].thisMonth) {
					if(day==0 || day==6 )
						...
						mCells[week][day] = new Cell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE,false,cellsData.get(tmp[week][day].day));
					}
				} else {
	...
	public void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		mWeekTitle.setBounds(
					WEEK_LEFT_MARGIN, WEEK_TOP_MARGIN, 
	    	...
	    	for(Cell[] week : mCells) {
				for(Cell day : week) {
					if(day.hitTest((int)event.getX(), (int)event.getY())) {
						mOnCellTouchListener.onTouch(day,event);
					}
				}
			}
    	}
	...
	protected void onDraw(Canvas canvas) {
		// draw background
		super.onDraw(canvas);
		//CalendarLinearLayout calendarLinearLayout = new CalendarLinearLayout(context);
		//calendarLinearLayout.setNote("eran", "eran");
		//View cll = this.findViewById(R.id.working_hours_container);
